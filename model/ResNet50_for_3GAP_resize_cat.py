...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, pretrained=False, zero_init_residual=False,
        ...
        self.out_conv2 = nn.Conv2d(512, 128, 1, 1, 0, bias=False)
        ...
        self.fc_last = nn.Linear(896, num_classes)
    ...
    def forward(self, x):
        ...
        feat2_conv = self.out_conv2(feat2)
        feat3_conv = self.out_conv3(feat3)
        feat4_conv = self.out_conv4(feat4)
        feat3_conv = F.interpolate(feat3_conv, size=feat2.size()[2:], mode='bilinear', align_corners=True)
        feat4_conv = F.interpolate(feat4_conv, size=feat2.size()[2:], mode='bilinear', align_corners=True)
        feat_conv = torch.cat([feat2_conv, feat3_conv, feat4_conv], dim=1)
...
def get_cam_threshold(module, pred, feat):
